...
def build_region_labels_for_tokens(
    ...
    for elem in norm_elements:
        ...
        x1 = max(0, int(x1_n * grid_size))
        y1 = max(0, int(y1_n * grid_size))
        x2 = min(grid_size, int(x2_n * grid_size) + 1)
        y2 = min(grid_size, int(y2_n * grid_size) + 1)
...
class FFEM(nn.Module):
    ...
    def _build_region_context(
        ...
        B, N, D = x.shape
        assert B == 1, "FFEM 当前仅支持 batch_size=1"
        ...
        for rid in range(1, max_region_id + 1):
            ...
            region_idx = min(rid - 1, self.num_regions - 1)
            gate = torch.sigmoid(
                region_logits[:, mask, region_idx].mean()
            ).view(1, 1, 1)

            pooled = pooled * gate
            region_ctx[:, mask, :] = pooled.expand(1, mask.sum().item(), D)
    ...
    def forward(
        ...
        rl_1d = region_labels.reshape(-1) if region_labels is not None else None
        region_ctx = self._build_region_context(x, rl_1d)

        kv_input = torch.cat([x, region_ctx], dim=-1)
        k = self.semantic_k_proj(kv_input)
        v = self.semantic_v_proj(kv_input)

        sem_out, _ = self.semantic_attn(query=x, key=k, value=v, need_weights=False)
        ...
        scale = torch.sigmoid(self.residual_scale) * 0.3
